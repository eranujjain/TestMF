...
**Quantity requirement: produce exactly {max_entity_types} entity types**.

**Hierarchy requirement (must include both specific and fallback types)**:

Your {max_entity_types} entity types must include the following layers:

A. **Fallback types (mandatory, place at the end of the list)**:
   - `Person`: Fallback for any individual not fitting a more specific person type.
   - `Organization`: Fallback for any organization not fitting a more specific organization type.

B. **Specific types ({specific_entity_count}, designed from the text)**:
   - Design more specific types for the main roles found in the text
...
class OntologyGenerator:
    ...
    def generate(
        ...
        system_prompt = (
            ...
            .replace('{max_entity_types}', str(Config.MAX_ENTITY_TYPES))
            .replace('{specific_entity_count}', str(max(Config.MAX_ENTITY_TYPES - 2, 1)))
            .replace('{max_edge_types}', str(Config.MAX_EDGE_TYPES))
        )
...
"""
        ...
        max_ent = Config.MAX_ENTITY_TYPES
        specific = max(max_ent - 2, 1)
        message += f"""
请根据以上内容，设计适合社会舆论模拟的实体类型和关系类型。

**必须遵守的规则**：
1. 必须正好输出{max_ent}个实体类型
2. 最后2个必须是兜底类型：Person（个人兜底）和 Organization（组织兜底）
3. 前{specific}个是根据文本内容设计的具体类型
