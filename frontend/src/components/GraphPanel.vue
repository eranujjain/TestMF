...
const wasSimulating = ref(false) // Track previous simulating state
...
const renderGraph = () => {
  ...
  // Process edges: count edges per node pair and index
  const edgePairCount = {}
  ...
  // Track edge index within each node pair
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Nodes whose self-loops are merged
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loops: one merged edge per node
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Full detail for each self-loop edge
        }
      })
      return
    }
    ...
    // Whether edge direction matches canonical order (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Curvature: spread multiple edges; straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
      // Even curvature spread for visibility
      // Wider curvature range when more edges between the pair
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if direction is reversed so bundles stay consistent
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation — node distance scales with parallel edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Distance from edge count between this pair
      // Base 150, +50 per extra parallel edge
    ...
    // Weak center pull so isolated groups drift toward middle
  ...
  const getLinkPath = (d) => {
    ...
    // Self-loop
    if (d.isSelfLoop) {
      // Arc leaving node and returning
      const loopRadius = 30
      // Start/end offsets on the right of the node
      const x1 = sx + 8  // Start offset
      const y1 = sy - 4
      const x2 = sx + 8  // End offset
      const y2 = sy + 4
      // Arc (sweep-flag=1 clockwise)
    ...
    // Control point — scales with edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset; larger share of distance when more parallel edges
  ...
  // Link label backgrounds (white for readability)
  ...
  // Keep refs for toggling visibility
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only; no sim restart (click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Real drag only after movement threshold
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // First real drag: restart simulation
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only ease sim stop after a real drag
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Edge label positions (horizontal, no rotation)
  ...
  // Click empty area closes detail panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
