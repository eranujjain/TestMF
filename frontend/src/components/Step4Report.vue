      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <button v-if="isComplete" class="next-step-btn" @click="goToInteraction">
            <span>Open deep interaction</span>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section content generated (section may still be in progress) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll so the button stays aligned
...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // Conversation / interview
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
  ...
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Graph stats
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Entity query
  }
}
...
// Parse functions (regexes use \u escapes for Chinese headings emitted by backend tools)
const PLACEHOLDER_NO_PLATFORM_REPLY = '\uff08\u8be5\u5e73\u53f0\u672a\u83b7\u5f97\u56de\u590d\uff09'
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          if (interview.redditAnswer && interview.redditAnswer !== PLACEHOLDER_NO_PLATFORM_REPLY) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== PLACEHOLDER_NO_PLATFORM_REPLY) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift queries → analysis sub-questions'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '\uff08\u8be5\u5e73\u53f0\u672a\u83b7\u5f97\u56de\u590d\uff09' || t === '(No reply on this platform)' || t === '[No reply]'
    }

    // Split answer text by question index
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question N:" (Chinese labels from backend)
      // 2. "1. " numbered lists (legacy)
      let matches = []
      let match
    ...
    // Answer text for question index
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // No split: full answer on first question only
      return qIdx === 0 ? answer : ''
    }
    
    // True if both platforms have real, different answers
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both non-placeholder and different
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle when both answers differ
                  ...
                  // Expand/collapse (hidden for placeholders)
...
const renderMarkdown = (content) => {
  ...
  // Strip leading ## title (shown outside)
  ...
  // Trim after list open
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Trim before list close
  ...
  // Trim br around blocks
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Fix p+br before blocks
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Collapse br
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Trim br after blocks
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Keep ol start attr for split lists
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Clear loading
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Scroll top if done, else follow logs
            if (isComplete.value) {
...
// Extract final answer markdown from LLM response
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Try <final_answer> tag
...
/* Console logs (aligned with Step3) */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
