<template>
  ...
  >
    <!-- Background: tech grid (only when projects exist or loading) -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <div class="card-files-wrapper">
          <!-- Corner accent — viewfinder style -->
        ...
        <!-- Card description (truncated requirement) -->
      ...
      <Transition name="modal">
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Not available for historical playback -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 (Start simulation) and Step 5 (Deep interaction) must be started while a run is active; historical playback is not supported</span>
            </div>
          </div>
        </div>
      </Transition>
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Cards in current row for centering
...
// Progress class from round counts
...
// Title from requirement (first 20 chars)
...
// Format simulation_id (first 6 chars of id)
...
// Navigate to graph build (Project)
const goToProject = () => {
...
// Navigate to analysis report (Report)
const goToReport = () => {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Record latest target state (even while animating)
        pendingState = shouldExpand
        
        // Clear previous debounce (new scroll intent wins)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // While animating, only record; apply after animation
        ...
        // Debounce expand/collapse to reduce flicker
        // Shorter delay when expanding (50ms), longer when collapsing (200ms)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Pending state may have been superseded
          ...
          // Unlock and flush any newer pending state
          setTimeout(() => {
            isAnimating = false
            
            // After animation, apply newer pending if any
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay before chaining
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Shrink bottom inset so expand needs more scroll
...
// Reload when keep-alive activates
onActivated(() => {
  loadHistory()
})
...
/* Simpler layout when no projects */
...
/* Square grid via CSS background */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor top-left; height grows downward without shifting grid */
...
.cards-container {
  ...
  /* min-height from JS by card count */
}
...
.status-icon:nth-child(2).available { color: #F59E0B; } /* Environment — amber */
...
.card-progress.completed { color: #10B981; }    /* Done — green */
...
/* Low-saturation palette — Morandi */
