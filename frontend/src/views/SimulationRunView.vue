...
// Read maxRounds from query on init so the child gets the value immediately
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Failed to close simulation environment, trying forced stop...`)
        try {
      ...
      // Environment not running; check whether we need to stop the process
...
const handleNextStep = () => {
  // Step3Simulation handles report generation and navigation directly; this is a fallback
  addLog('Proceeding to Step 4: report generation')
}
...
onMounted(() => {
  ...
  // Log maxRounds (already read from query on init)
  if (maxRounds.value) {
