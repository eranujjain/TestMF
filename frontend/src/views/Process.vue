            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After upload, an LLM analyzes the documents and generates an ontology suited to opinion simulation (entity types + relation types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API description</div>
                <div class="detail-content">
                  Using the generated ontology, documents are chunked and the Zep API builds the knowledge graph and extracts entities and relations.
                </div>
              </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files. Return to home and try again.'
    loading.value = false
    return
  }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Auto-start graph build if needed
      if (response.data.status === 'ontology_generated' && !response.data.graph_id) {
        await startBuildGraph()
      }
      
      // Resume polling for in-flight build
